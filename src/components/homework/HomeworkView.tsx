    ...
    <div className="rounded-2xl border border-ui bg-white p-4 space-y-3">
      <div className="flex items-center justify-between gap-3">
        ...
        {(userRole === 'parent' || userRole === 'coparent') && (
          <button onClick={() => setShowAdd(true)} className="flex min-h-11 items-center gap-1.5 rounded-xl bg-ui-primary px-3 py-2 text-sm font-semibold text-white transition-colors hover:bg-ui-primary/90">
            <Plus size={14} /> Add
          </button>
        )}
      </div>
        ...
        {(userRole === 'kid' ? pendingHomework : visibleHomework).map((item) => {
          ...
          return (
            <div key={item.id} className={`flex flex-col gap-3 rounded-xl border p-3 sm:flex-row sm:items-center sm:justify-between ${userRole === 'kid' ? 'border-2 border-ui-soft bg-white shadow-sm' : 'border-ui'}`}>
              ...
              <div className={`flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center ${userRole === 'kid' ? 'sm:justify-end' : ''}`}>
                <button
                  ...
                  className={`rounded-xl text-xs font-bold uppercase tracking-wide border ${
                    userRole === 'kid' ? 'min-h-11 px-4 py-3 sm:min-w-[150px]' : 'min-h-10 px-3 py-2'
                  } ${
                ...
                {(userRole === 'parent' || userRole === 'coparent') && (
                  <>
                    <button
                      ...
                      disabled={isHomeworkPending(item.id)}
                      className="min-h-10 rounded-lg p-2 text-blue-600 hover:bg-blue-50"
                    ...
                    <button
                      ...
                      disabled={isHomeworkPending(item.id)}
                      className="min-h-10 rounded-lg p-2 text-rose-600 hover:bg-rose-50"
                    >
                      <Trash2 size={14} />
                    </button>
                  </>
        ...
        {userRole === 'kid' && completedHomework.length > 0 && (
          <div className="pt-2 space-y-2">
            ...
            {completedHomework.map((item) => (
              <div key={`done-${item.id}`} className="flex flex-col gap-3 rounded-xl border-2 border-emerald-200 bg-emerald-50 p-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                ...
                <button
                  ...
                  disabled={isHomeworkPending(item.id)}
                  className="min-h-11 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-xs font-bold uppercase tracking-wide text-rose-700 sm:min-w-[150px]"
                >
          ...
          <div className="w-full max-w-md bg-white rounded-2xl border border-ui p-5 space-y-3" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 rounded-xl bg-ui-primary py-2 font-semibold text-white transition-colors hover:bg-ui-primary/90"
              >
                Save
              </button>
            </div>
