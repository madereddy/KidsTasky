...
export function TaskCard({
  ...
  const accentColor = isDone
    ? 'border-emerald-500'
    : (isLocked
        ? (darkMode ? 'border-ui-dark' : 'border-ui')
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        accentColor,
        isDone
          ? (darkMode ? 'opacity-60 bg-emerald-500/5 border-emerald-500/30' : 'opacity-70 bg-emerald-50 shadow-sm')
          : (isLocked
              ? (darkMode ? 'bg-ui-deep grayscale' : 'bg-ui-soft grayscale')
        ...
        <motion.div
          ...
          className={cn(
            ...
            isDone
              ...
              : (isLocked
                  ? (darkMode ? 'bg-ui-dark border-ui-dark text-ui-secondary' : 'bg-ui-soft-2 border-ui text-ui-muted-2')
                  : (urgency === 'overdue'
                      ? (darkMode ? `bg-rose-500/20 text-rose-500 border-rose-500/50${isWallMode ? '' : ' animate-pulse'}` : 'bg-red-50 text-red-500 border-red-200')
                      : (urgency === 'soon'
                          ? (darkMode ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' : 'bg-sky-50 text-sky-500 border-sky-100')
                          : (darkMode ? 'bg-ui-dark text-ui-muted-2 border-ui-dark hover:border-ui-dark-2 hover:text-ui-secondary shadow-lg' : 'bg-white text-ui-secondary border-ui shadow-sm hover:border-sky-300')))),
          )}
        >
          <AnimatePresence mode="wait">
            <motion.span
              key={isDone ? 'done' : (isPending ? 'saving' : (isLocked ? 'locked' : 'pending'))}
          ...
          <motion.button
            ...
            className={cn(
              ...
              isLocked || isPending
                ...
                : urgency === 'overdue'
                  ? (darkMode ? 'bg-amber-500 text-ui-primary shadow-[0_0_20px_rgba(245,158,11,0.4)]' : 'bg-red-50 text-red-500 hover:bg-red-100')
                  : (darkMode ? 'bg-blue-600/20 border border-blue-500/50 text-blue-400 hover:bg-blue-600/40' : 'bg-sky-50 text-sky-600 hover:bg-sky-100 hover:text-sky-700'),
            )}
          >
            <span className="relative z-10 flex items-center justify-center gap-2">
              {isPending ? 'Saving...' : (isLocked ? <><Lock className="w-4 h-4" /> {themeVocab?.locked || 'Locked'}</> : (themeVocab?.markDone || 'Mark Done'))}
            </span>
          </motion.button>
          {onSkip && !isLocked && !isPending && (
            <button
              type="button"
