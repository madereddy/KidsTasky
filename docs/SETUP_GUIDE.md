...
*   **`GOOGLE_CLIENT_ID`** & **`GOOGLE_CLIENT_SECRET`**: Credentials you get from the Google Cloud Console. Used to enable Google Calendar 2-way sync.
...
## Setting Up Google OAuth (For Calendar Sync)

KidTasker allows families to connect their Google Calendars so that events natively sync to the Family Dashboard. The backend gracefully handles multiple OAuth tokens linked to your Family's `parentId`, meaning different parents or members can connect their individual Google accounts, and the background worker will sync all associated events into the unified family view.
...
3.  **Configure the OAuth Consent Screen:**
    ...
    *   Add the scopes needed: `.../auth/calendar.readonly` or `.../auth/calendar` depending on the sync direction you want (KidTasker primarily requests read access to ingest events).
